 ...
 * \defgroup emailnewsletter EmailNewsletterApplication
 *
...
private:
	...
	/**
	 * \brief Send a mail packet
	 */
	void SendMail(void);
	
	/**
	 * \brief Send remaining mail data if the mail couldn't be transmitted in one packet
	 */
	void SendRemaining(void);
	
	/**
	 * \brief Schedule the next mail simulating the time delay of smtp chatter
	 */
	void ScheduleNextMail(void);
	
	/**
	 * \brief Check if mails per server has reached and simulate TCP (dis-)connect time delay
	 */
	 ...
	 */
	void BufferAvailableCb(Ptr<Socket>, uint32_t); // for socket's SetSendCallback
};
